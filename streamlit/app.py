"""
Streamlit Dashboard for Travel Recommendation System
...
st.sidebar.title("✈️ Travel ML Dashboard")
st.sidebar.markdown("---")
...
def flight_prediction_page():
    ...
    if st.button("Predict Price", type="primary"):
        if models['flight'] is not None:
            ...
            st.warning("Flight price model not loaded. Please train the model first.")
...
def gender_classification_page():
    ...
    if st.button("Predict Gender", type="primary"):
        if models['gender'] is not None:
            ...
            st.warning("Gender classifier not loaded. Please train the model first.")
